...
extern char passward[32];
extern char wifissid[32];
extern int wifi_connected;

static void write_config_txt(char * ssid, char * password)
{
    ...
    char buf[64] = {0};
	int pos = 0;
	pos += sprintf(&buf[pos], "%s %s", ssid, password);
    fwrite(buf, 1, pos, f);
    fclose(f);

    // Display the read contents from the file
    printf("\r\nwrite from config.txt: %s\r\n", buf);
}
                                ...
                                int32_t event_id, void* event_data)
{
    ...
    case IP_EVENT_STA_GOT_IP: {
		wifi_connected = true;
		printf("event_handler wifi_connected \r\n");
...
//		write_config_txt(wifissid, passward);
...
static void initialise_wifi(void)
{
	...
	sprintf((char *)wifi_config.sta.ssid, "%s", wifissid);
  	sprintf((char *)wifi_config.sta.password, "%s", passward);
	printf("wifissid %s  passward %s \r\n", wifissid, passward);
...
//		for(int i = 0; i < strlen((char *)sta_config.sta.ssid); i ++){
//			printf("%02x ", sta_config.sta.ssid[i]);
//		}
//		printf("\r\n");
		strcpy(wifissid, (char *)sta_config.sta.ssid);
        break;
...
//		for(int i = 0; i < strlen((char *)sta_config.sta.password); i ++){
//			printf("%02x ", sta_config.sta.password[i]);
//		}
//		printf("\r\n");
		strcpy(passward, (char *)sta_config.sta.password);
		printf("ssid %s  password %s\r\n", wifissid, passward);
		write_config_txt(wifissid, passward);
...
void start_blufi(void)
{
	...
	while(strlen(wifissid) == 0) vTaskDelay(100/ portTICK_PERIOD_MS);
